...
#include <getopt.h>
#ifdef _JBTYPE
typedef _JBTYPE sigjmp_buf[_JBLEN+1+((128  + sizeof (sigset_t) - 1)
                     /sizeof (_JBTYPE))];
#else
typedef int sigjmp_buf[_JBLEN+1+(128 /sizeof (int))];
#endif
...
//IF_FEATURE_VI_SEARCH(last_search_pattern = xzalloc(2);) \ // commented due to lack ENABLE_FEATURE_VI_READONLY support, from macro abovea
